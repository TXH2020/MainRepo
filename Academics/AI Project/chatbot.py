...
with open('once.txt','a') as f:
    f.write('1')
with open('once.txt','r') as f:
    if(f.read()=='1'):
        nltk.download('punkt')
interpreter = tflite.Interpreter(model_path=r'path-to-model.tflite')
interpreter.allocate_tensors()
with open(r'path to intent.pkl','rb') as f:
  intents=pickle.load(f)
with open(r'path to class.pkl','rb') as f:
  classes=pickle.load(f)
with open(r'word.pkl','rb') as f:
  words=pickle.load(f)
...
def response(sentence, userID='123', show_details=False):
    ...
    if results:
        ...
        while results:
            ...
            results.pop(0)
print(response('what kind of mopeds do you rent?'))
